...
Zomoc은 페이지네이션된 API 응답을 지능적으로 모의할 수 있습니다. 특정 엔드포인트에 `pagination` 구성이 제공되면, Zomoc은 요청(쿼리 파라미터 또는 본문)에서 페이지와 크기를 읽어 요청된 만큼의 아이템을 정확하게 생성합니다.
...
이 설정은 항상 정확히 5개의 모의 `ITag` 아이템을 가진 배열을 반환합니다.
...
### 모의 전략: 고정(Fixed) vs. 무작위(Random)

인터페이스의 `union` 타입(`'a' | 'b'`)이나 `enum` 타입을 위해 `zomoc`이 모의 데이터를 생성하는 방식을 제어할 수 있습니다. 이는 예측 가능한 테스트를 작성할 때 특히 유용합니다.
...
Zomoc은 강력한 도구이지만, 사용하기 전에 알아두어야 할 몇 가지 한계점이 있습니다.
