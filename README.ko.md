...
Zomoc은 페이지네이션 API 응답을 지능적으로 모킹할 수 있습니다. 특정 엔드포인트에 `pagination` 설정을 추가하면, Zomoc은 요청(쿼리 파라미터 또는 본문)에서 페이지 번호와 사이즈를 읽어 정확히 요청된 개수만큼의 아이템을 생성합니다.

**1. `mock.json`에 `pagination` 객체를 설정합니다.**

`mock.json` 파일에서, 단순한 타입 이름 문자열 대신 `responseType`과 `pagination` 키를 가진 객체를 사용하세요.
...
- `totalKey`: 전체 아이템 개수를 담고 있는 키 (예: `total`)
- `pageKey`: 요청의 쿼리 파라미터 또는 본문에서 페이지 번호를 나타내는 키
- `sizeKey`: 요청의 쿼리 파라미터 또는 본문에서 페이지 크기를 나타내는 키
...
> **참고**: 페이지네이션이 설정된 엔드포인트는 요청된 `size`만큼 아이템을 생성합니다. 반면 `IUser[]`와 같이 페이지네이션이 설정되지 **않은** 일반 배열 응답의 경우, 데이터가 좀 더 현실감 있게 느껴지도록 1~3개 사이의 랜덤한 개수의 아이템을 자동으로 생성합니다.
